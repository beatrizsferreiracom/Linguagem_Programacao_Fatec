//Tabuada com For

//Declaração das bibliotecas
#include<stdio.h>
#include<conio.h>

//Procedimento Principal main ()

main()
{
    //Declaração das Variáveis com o modificador unsigned
    //Ou seja, não permite valores negativos
    unsigned int t, c, vl;

    printf("Tabuada: ");
    scanf("%u", &t);

    //A Estrutura de Repti��o - Para
    for(vl = 0; vl <= 10; vl++)
    {
        //Processamento de Dados
        c = t * vl;

        //Mostrar os dados na tela, aqui somente o valor da variável t
        printf("%u", t);

        //Mostrar os dados na twla, aqui mostra o texto X mais o valor da vari�vel vl
        printf(" x %u", vl);

        //Mostrar os dados na tela, aqui mostra o texto = mais o valor da vari�vel c e quebra a linha
        printf(" = %u\n", c);
    }
    //Segura o programa no ar
    getch();
}
